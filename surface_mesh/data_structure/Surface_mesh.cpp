...
namespace graphene {
	namespace surface_mesh {
			...
			Surface_mesh()
		{
			...
			fvconn_ = add_feature_v_property<FeatureVertex_connectivity>("v:feature connectivity");
			fhconn_ = add_feature_h_property<FeatureHalfedge_connectivity>("h:feature connectivity");
			flconn_ = add_line_property<FeatureLine_connectivity>("l:feature connectivity");
			epconn_ = add_endpoint_property<EndPoint_connectivity>("v:end point connectivity");
			vpoint_ = add_vertex_property<Point>("v:point");
			fpoint_ = add_feature_v_property<Point>("f:point");   //feature point
			...
			//deleted feature property
			ldeleted_ = add_line_property<bool>("l:deleted", false);
			lvisual_ = add_line_property<bool>("l:is visual", true);
			fvdeleted_ = add_feature_v_property<bool>("fv:deleted", false);
			fedeleted_ = add_feature_e_property<bool>("fe:deleted", false);
			epdeleted_ = add_endpoint_property<bool>("ep:deleted", false);

			deleted_vertices_ = deleted_edges_ = deleted_faces_ = deleted_feature_edges_ = deleted_feature_vertices_ = deleted_lines_ = 0;
			garbage_ = false;
		}
			...
			operator=(const Surface_mesh& rhs)
		{
			if (this != &rhs)
			{
				...
				fprops_ = rhs.fprops_;
				//feature property
				fvprops_ = rhs.fvprops_;
				fhprops_ = rhs.fhprops_;
				feprops_ = rhs.feprops_;
				lprops_ = rhs.lprops_;
				...
				//feature
				fpoint_ = feature_v_property<Point>("v:feature point");
				fvconn_ = feature_v_property<FeatureVertex_connectivity>("v:feature connectivity");
				fhconn_ = feature_h_property<FeatureHalfedge_connectivity>("h:feature connectivity");
				...
				deleted_feature_edges_ = rhs.deleted_feature_edges_;
				deleted_feature_vertices_ = rhs.deleted_feature_vertices_;
			...
			assign(const Surface_mesh& rhs)
		{
			if (this != &rhs)
			{
				...
				fhconn_ = add_feature_h_property<FeatureHalfedge_connectivity>("h:feature connectivity");
				vpoint_ = add_vertex_property<Point>("v:point");
				fpoint_ = add_feature_v_property<Point>("f:point");
				vdeleted_ = add_vertex_property<bool>("v:deleted", false);
				edeleted_ = add_edge_property<bool>("e:deleted", false);
				fdeleted_ = add_face_property<bool>("f:deleted", false);
				...
				//feature 
				fpoint_.array() = rhs.vpoint_.array();
				fvconn_.array() = rhs.fvconn_.array();
				fhconn_.array() = rhs.fhconn_.array();
				...
				fvprops_.resize(rhs.fvertices_size());
				fhprops_.resize(rhs.fhalfedges_size());
				feprops_.resize(rhs.fedges_size());
				lprops_.resize(rhs.flines_size());
		...
		bool
			Surface_mesh::
			read(const std::string& filename, const std::string& ext)
		{
			if (ext == "fld")
			{
				return read_feature(*this, filename);
			}
			else if (ext == "clr")
			{
				return read_segmentation(*this, filename);
			}
			return read_mesh(*this, filename, ext);
		}
			...
			clear()
		{
			...
			eprops_.free_memory();
			fprops_.free_memory();
			fvprops_.free_memory();
			feprops_.free_memory();
			fhprops_.free_memory();
			lprops_.free_memory();
			epprops_.free_memory();

			deleted_vertices_ = deleted_edges_ = deleted_faces_ = deleted_feature_edges_ = deleted_feature_vertices_ = deleted_lines_ = deleted_end_point_ = 0;
			garbage_ = false;
		}
		...
		void
			...
			reserve(unsigned int nvertices,
				unsigned int nedges)
		{
			fvprops_.reserve(nvertices);
			fhprops_.reserve(2 * nedges);
			feprops_.reserve(nedges);
		}
		...
		//-----------------------------------------------------------------------------
		Surface_mesh::FeatureVertex
			Surface_mesh::
			add_feature_vertex(const Point& p)
		{
			FeatureVertex v = new_feature_v();
			fpoint_[v] = p;
			return v;
		}
		...
		void 
			Surface_mesh::
			exchange_feature_vertex(const FeatureVertex v, const Point p)
		{
			fpoint_[v] = p;
		}
		...
		Surface_mesh::FeatureVertex 
			Surface_mesh::
			find_shortest_distance_point(FeatureLine l, Point v)
		{
			Scalar dis, min(1000);
			...
			vhead = head(l);
			vend = tail(l);
			dis = dist(fpoint_[vhead], v);
			if (dis < min)
			{
				min = dis;
				temp = vhead;
			}
			while (to_vertex(*hit) != vend)
			{
				vit = to_vertex(*hit);
				dis = dist(fpoint_[vit], v);
				if (dis < min)
				{
					min = dis;
					temp = vit;
			...
			/*else 
			{
				Surface_mesh::FeatureHalfedge hs;
				hs = halfedge(temp);
				int k(0);
				while (to_vertex(hs) != vend)
				{
					k++;
				}
				if (k <= 3)
				{
					if (get_exten(l) != 2 && get_exten(l) != 0)
					{
					}
				}
				k = 0;
				while (from_vertex(hs) != vhead)
				{
					k++;
				}
			}*/
		...
		//-----------------------------------------------------------------------------
		Surface_mesh::FeatureHalfedge
			Surface_mesh::
			find_halfedge(FeatureVertex start, FeatureVertex end) const
		{
			assert(is_valid(start) && is_valid(end));

			FeatureHalfedge h = halfedge(start);
			const FeatureHalfedge hh = h;
		...
		void
			Surface_mesh::
			update_featureLine( FeatureLine l,  Vertex v, bool is_head)
		{
			/*FeatureHalfedge h;
			if (is_head)
			 h = insert_head_vertex(l, v_);
			else
			 h = insert_tail_vertex(l, v_);*/
			//set_line(h, l, f.idx());
			update_lineporp(l, v, is_head);
			//update_epporp(ev, h, v, v_, f);
		}
		//-----------------------------------------------------------------------------

		Surface_mesh::EndPoint 
			Surface_mesh::
			add_end_point(FeatureLine l, FeatureHalfedge h, FeatureVertex v, Face f, bool is_head)
		{
			EndPoint ev(new_end_point());
			set_epporp(ev, l, v, f, is_head);
			return ev;
		}

		//-----------------------------------------------------------------------------
		Surface_mesh::FeatureLine
			Surface_mesh::
			add_feature_line(const std::vector<FeatureVertex>& vertices, const std::vector<int>& face_id,Scalar length,bool is_ridge)
		{

			const unsigned int n(vertices.size());
			assert(n >= 2);
			NextCache1& next_cache = add_line_next_cache_;
			std::vector<FeatureHalfedge>   halfedges = add_feature_halfedges_;
			unsigned int i, ii, id,num_h;
			FeatureVertex v;
			FeatureHalfedge  inner_next, inner_prev, outer_next, outer_prev, boundary_next, boundary_prev, patch_start, patch_end;

			halfedges.clear();
			halfedges.reserve(n-1);
			num_h = n - 1;
			next_cache.clear();
			next_cache.reserve(n);
			
			for (i = 0, ii = 1; i < num_h; ++i, ++ii)
				halfedges.push_back(find_halfedge(vertices[i], vertices[ii]));

			for (i = 0, ii = 1; i < num_h; ++i, ++ii)
				halfedges[i] = new_feature_edge(vertices[i], vertices[ii]);

			FeatureLine l(new_faeture_line());
			set_lineporp(l, halfedges[0],vertices[0],vertices[n-1],length,is_ridge);
			set_line_num(l, num_h);
			int k = halfedges.size();
			if (k == 1)
			{
				v = vertices[0];
				set_halfedge(v, halfedges[0]);
				set_line(halfedges[0], l, face_id[0]);
				return l;
			}
			else
			{
				for (i = 0, ii = 1; i < num_h - 1; ++i, ++ii)
				{
					v = vertices[ii];
					...
					{
						set_halfedge(v, inner_prev);
						next_cache.push_back(NextCacheEntry1(outer_prev, outer_next));
						next_cache.push_back(NextCacheEntry1(inner_prev, inner_next));
					}
					// set line handle
					set_line(halfedges[i], l, face_id[i]);
				}
				v = vertices[ii];
				inner_prev = halfedges[i];
				set_halfedge(v, inner_prev);
				...
				NextCache1::const_iterator ncIt(next_cache.begin()), ncEnd(next_cache.end());
				for (; ncIt != ncEnd; ++ncIt)
				{
		...
		Surface_mesh::FeatureHalfedge 
			Surface_mesh::
			tail_halfedge(FeatureLine l)
		{
			FeatureVertex tail_;
			tail_ = tail(l);
			return halfedge(tail_);
		}

		//-----------------------------------------------------------------------------
		Surface_mesh::FeatureHalfedge
			Surface_mesh::
			insert_head_vertex(FeatureLine l, FeatureVertex v, int  face_id)
		{
			FeatureHalfedge  inner_next, inner_prev, outer_next, outer_prev;
			Surface_mesh::FeatureVertex vhead;
			inner_next = get_halfedge(l);
			Surface_mesh::FeatureVertex v1, v2;
			v1 = from_vertex(inner_next);
			v2 = to_vertex(inner_next);
			outer_prev = opposite_halfedge(inner_next);
			vhead = head(l);
			inner_prev = new_feature_edge(v, vhead);
			outer_next = opposite_halfedge(inner_prev);
			set_next_halfedge(inner_prev, inner_next);
			set_next_halfedge(outer_prev,outer_next);
			set_halfedge(v, inner_prev);
			set_line_head(l, inner_prev, v);
			set_line(inner_prev, l, face_id);
			set_line_num(l, get_line_num(l) + 1);
			return inner_prev;
		}
		...
		Surface_mesh::FeatureHalfedge
			Surface_mesh::
			insert_tail_vertex(FeatureLine l, FeatureVertex v, int face_id)
		{
			FeatureHalfedge head_halfedge, inner_next, inner_prev, outer_next, outer_prev;
			Surface_mesh::FeatureVertex vtail;
			vtail = tail(l);
			inner_prev = tail_halfedge(l);
			outer_next = opposite_halfedge(inner_prev);
			
			inner_next = new_feature_edge(vtail, v);
			outer_prev = opposite_halfedge(inner_next);
			set_next_halfedge(inner_prev, inner_next);
			set_next_halfedge(outer_prev, outer_next);
			set_halfedge(v, inner_next);
			set_line_tail(l, v);
			set_line(inner_next, l, face_id);
			set_line_num(l, get_line_num(l) + 1);
			return inner_next;
		}
		...
		Surface_mesh::FeatureHalfedge 
			Surface_mesh::
			add_feature_halfedge(FeatureVertex start, FeatureVertex end)
		{
			return new_feature_edge(start,end);
		}

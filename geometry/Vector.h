...
#ifndef GRAPHENE_VECTOR_H
#define GRAPHENE_VECTOR_H
...
template <typename Scalar>
inline Scalar dist(const Vector<Scalar, 3>& v0, const Vector<Scalar, 3>& v1)
{
	Scalar s = (v0.x-v1.x)*(v0.x - v1.x);
	s += (v0.y-v1.y)*(v0.y - v1.y);
	s += (v0.z-v1.z)*(v0.z - v1.z);
	return sqrt(s);
}

template <typename Scalar>
inline Scalar vectorangle(const Vector<Scalar, 3>& v0, const Vector<Scalar, 3>& v1)
{
	Scalar s = v0.x*v1.x;
	s += v0.y*v1.y;
	s += v0.z*v1.z;
	Scalar a = sqrt(v0.x*v0.x + v0.y*v0.y + v0.z*v0.z);
	Scalar b = sqrt(v1.x*v1.x + v1.y*v1.y + v1.z*v1.z);
	return s/(a*b);
}
